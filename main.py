...
if __name__ == "__main__":
    ...
    MO = TrainOptuna(name=name, causal=causal, epoch=epoch, device=device, score_func='K2', white_list=white_list, init_edges=init_edges, remove_init_edges=remove_init_edges)
    best_values = MO.run(number_of_trials=number_of_trials)
    kwargs= {'train': MO.train_dataset, 'test': MO.test_dataset, 'val':MO.val_dataset}
